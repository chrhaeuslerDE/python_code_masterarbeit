...
dataframeTest = pandas.DataFrame(pandas.read_csv('train.txt', sep = ' '))
...
def wider_model():
    # create model
    model = Sequential()
    model.add(Dense(20, input_dim=20, kernel_initializer='normal', activation='tanh'))
...
#Modell erzeugen
model = KerasRegressor(build_fn=wider_model, epochs=250, batch_size=50, verbose=0)

for x in range(5):
    #Loop einfügen, um Optimizer mehrere Startpunkte zu geben!?
    his = model.fit(X_train,y_train, callbacks=[history], shuffle=True)
